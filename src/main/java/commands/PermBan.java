...
public class PermBan extends ListenerAdapter {
    EventWaiter waiter = new EventWaiter();

    public PermBan(EventWaiter waiter) {
        this.waiter = waiter;
    }
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (spliced[0].equalsIgnoreCase(prefix + "permban")) {
                    ...
                    if (length == 2) {
                        ...
                        if (spliced[1].startsWith("<@")) {
                            ...
                            if (mentionedMember.hasPermission(Permission.BAN_MEMBERS)) {
                                ...
                                channel.sendMessage("Are you sure? Type 'yes' in this channel to confirm this permban.")
                                        .queue();
                                ;
                                initWaiter(channelID, user, event.getJDA().getShardManager(), mentionedMember,
                                        textChannel, reason, channel);
                            }

                        } else {
                            channel.sendMessage("Can you use the command correctly PLEASE? Just do " + prefix
                                    + "permban @<user> [reason]! SIMPLE!").queue();
                        }

                    }
                } else {
                    channel.sendMessage("This command will permanently ban someone. To use it, just do " + prefix
                            + "permban @<user> [reason]! ").queue();
                }
            } else {
    ...
    private void initWaiter(long channel, long user, ShardManager shardmanager, Member member, TextChannel log,
                            String reason, MessageChannel sendchannel) {
        waiter.waitForEvent(MessageReceivedEvent.class, (event) -> {
            ...
            info.setTitle("NEW PERMBAN");
            info.addField("Info:", member.getEffectiveName() + " was banned by " + event.getMember().getEffectiveName(),
                    false);
            ...
            event.getChannel().sendMessage("Ban successful! :white_check_mark:").queue();
